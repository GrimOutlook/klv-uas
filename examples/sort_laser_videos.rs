...
use klv_uas::klv_value::KlvValue;
use klv_uas::tag::Tag;
use tracing::debug;
use tracing::error;
use tracing::info;
...
fn main() -> ExitCode {
    ...
    let ts_files = fs::read_dir(directory)
        .unwrap()
        .map(|dr| dr.unwrap())
    ...
    debug!(
        "Transport stream files found: {:?}",
        ts_files.iter().map(|f| f.file_name()).collect_vec()
    );
    ...
    for video in ts_files.iter() {
        ...
        // Set the laser video variable to false. Only move it to the laser
        // folder if the laser is seen firing.
        let mut is_laser_video = false;

        // Boilerplate to create a TSReader object
        let f = File::open(filename).expect("Couldn't open file");
        let mut reader = TsReader::new(BufReader::new(f))
        ...
        loop {
            // Check to see if any of the KLV data indicates that the laser is
            // on
            ...
            let Some(klv) = KlvPacket::from_bytes(payload).unwrap() else {
                continue;
            };
        ...
        let rename_directory = if is_laser_video {
            // If the laser is ever on move the video into the laser-video
            // folder.
            laser_folder
        } else {
            // If no laser is ever seen move the video into the non-laser-video
            // folder.
            non_laser_folder
        };
        ...
        match fs::rename(
            ...
            format!(
                "{}/{}",
                rename_directory,
                video.file_name().into_string().unwrap()
            ),
        ) {
            Ok(_) => (),
            Err(e) => {
                error!(
                    "Cannot move video [{}] into the folder [{}] due to error: {}",
                    filename, laser_folder, e
                )
            }
        };
    }
